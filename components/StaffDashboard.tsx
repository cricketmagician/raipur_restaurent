...
interface StaffDashboardProps {
    hotelSlug: string;
    department: 'reception' | 'kitchen' | 'housekeeping' | 'waiter';
    allowedTypes: string[];
    title: string;
...
export function StaffDashboard({ hotelSlug, department, allowedTypes, title, icon }: StaffDashboardProps) {
    ...
    useEffect(() => {
        ...
        const hasWater = departmentPending.some(r => r.type === "Mineral Water" || r.type === "Water");
        const hasWaiterCall = departmentPending.some(r => r.type === "Waiter Call");
                                    ...
                                    <tr key={req.id} className={`transition-all group ${getRowStyle(req.type)}`}>
                                        ...
                                        <td className="p-6">
                                            <div className="flex items-center font-black text-slate-900 mb-1">
                                                {req.type.toLowerCase().includes('waiter') || req.type.toLowerCase().includes('dining') ? <Utensils className="w-4 h-4 mr-2 text-amber-500" /> : <Bell className="w-4 h-4 mr-2 text-blue-500" />}
                                                {req.type}
                                            </div>
